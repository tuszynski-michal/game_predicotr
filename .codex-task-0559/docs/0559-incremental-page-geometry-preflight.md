...
- [x] Nowy job przypina kompatybilny manifest, preferując exact v3 przed v2→v3.
...
API zapisuje opcjonalny deskryptor `basePageGeometryManifest` w input payload i kluczu idempotencji. Deskryptor przypina job, checksumę manifestu, checksumę source manifestu i tryb `exact_policy` lub `lateral_v2_to_v3`. Worker weryfikuje manifest fail-closed.
...
- API wybiera exact v3 przed v2→v3; brak kandydata uruchamia pełny skan.
...
- API wybiera i przypina najnowszy zgodny manifest z pierwszeństwem exact v3.
- Worker planuje reuse v3→v3 i v2→v3, unieważnia zmienione override'y oraz

...
_COMPATIBILITY_MODES = frozenset(
    {EXACT_POLICY_COMPATIBILITY, LATERAL_V2_TO_V3_COMPATIBILITY}
)
...
def load_base_manifest(
    ...
    *,
    ...
    if (
        ...
        or descriptor.source_manifest_checksum_sha256
        != source_manifest_checksum_sha256
...
def plan_manifest_reuse(
    ...
    *,
    ...
    for checksum, value in overrides.items():
        base_entry = raw_entries.get(checksum)
        if (
            isinstance(checksum, str)
            and isinstance(value, Mapping)
            and (
                not isinstance(base_entry, Mapping)
                or not _manual_entry_matches_override(base_entry, value)
            )
        ):
            changed_override_checksums.add(checksum)
    ...
    for original in originals:
        ...
        if isinstance(anchor_checksum, str) and anchor_checksum in changed_override_checksums:
            recompute.append(original)
            continue
...
class PageGeometryCheckpointStore:
    ...
    def _safe_data_path(self, relative_path: str) -> Path:
        ...
        path = (
            self._artifact_root / Path(*PurePosixPath(relative_path).parts)
        ).resolve()
...
def _manual_entry_matches_override(entry: Mapping[str, object], value: object) -> bool:
    return (
        ...
        and entry.get("manualOverrideDecisionChecksumSha256")
        == value.get("decisionChecksumSha256")
...
def _lateral_v2_to_v3_compatible(base: object, target: object) -> bool:
    return (
        ...
        and base.get("partialGridTrainingProfile")
        == target.get("partialGridTrainingProfile")
    )

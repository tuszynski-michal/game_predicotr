...
> {
  try {
    ...
    if (result.error !== undefined || !(result.data instanceof Blob)) {
      return {
        error: apiErrorMessage(
          result.error,
          'Nie udaÅ‚o siÄ™ wygenerowaÄ‡ podglÄ…du poprawionej siatki.',
        ),
        isRevisionConflict:
    ...
    return {
      error: 'PoÅ‚Ä…czenie z lokalnym Admin API zostaÅ‚o przerwane.',
  ...
  try {
    ...
    if (result.error !== undefined || result.data === undefined) {
      return {
        error: apiErrorMessage(
          result.error,
          'Nie udaÅ‚o siÄ™ zapisaÄ‡ poprawionej geometrii.',
        ),
        isRevisionConflict:
          isApiErrorCode(result.error, 'IMAGE_REVIEW_REVISION_CONFLICT') ||
          isApiErrorCode(

...
export function OperationalReviewGeometryEditor({
  ...
  useEffect(() => {
    ...
    image.onerror = () => {
      setLoadingSource(false);
      setError('Nie udaÅ‚o siÄ™ wczytaÄ‡ oryginalnego obrazu.');
    };
    image.src = sourceUrl;
    return () => {
      image.onload = null;
    ...
    if (!result.ok) {
      setError(
        result.isRevisionConflict
          ? `${result.error} Zamknij edytor i przeÅ‚aduj planszÄ™.`
          : result.error,
      );
      return;
    }
  ...
  async function saveGeometry() {
    ...
    if (!result.ok) {
      setError(
        result.isRevisionConflict
          ? `${result.error} PrzeÅ‚aduj planszÄ™ przed kolejnÄ… korektÄ….`
          : result.error,
      );
      return;
    }
    ...
    <>
      ...
      >
        <header>
          <div>
            ...
            <h2 id="operational-review-geometry-title">
              Ustaw cztery naroÅ¼niki planszy
            </h2>
            <p>
              KolejnoÅ›Ä‡: lewy gÃ³rny, prawy gÃ³rny, prawy dolny, lewy dolny.
            </p>
          </div>
          <button
        ...
        <div className="operationalReviewGeometryBody">
          <section>
            <h3>OryginaÅ‚ i ukoÅ›na siatka 5 Ã— 3</h3>
            {loadingSource ? <p>Wczytywanie obrazuâ€¦</p> : null}
            <canvas
              aria-label="Oryginalny obraz z edytowalnÄ… siatkÄ…"
          ...
          <section className="operationalReviewGeometryPreview">
            <div>
              ...
              <button
                ...
                type="button"
              >
                {loadingPreview ? 'Generowanieâ€¦' : 'Aktualizuj podglÄ…d'}
              </button>
            </div>
            {previewUrl === null ? (
              <p className="operationalReviewGeometryPlaceholder">
                Wygeneruj podglÄ…d przed zapisem.
              </p>
            ) : (
              <>
                ...
                <div
                  aria-label="PodglÄ…d 15 nowych cropÃ³w"
        ...
        <footer>
          <span>
            Zapis utworzy nowÄ… rewizjÄ™ i ponownie otworzy wybÃ³r symboli.
          </span>
          <div>
            <button
              className="secondaryButton"
            ...
            <button
              ...
              type="button"
            >
              {saving ? 'Zapisywanieâ€¦' : 'Zapisz nowÄ… rewizjÄ™'}
            </button>
          </div>
        </footer>
      </dialog>

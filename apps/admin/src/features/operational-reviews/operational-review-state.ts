...
export type OperationalReviewKeyboardAction =
  ...
  | { readonly type: 'submit-confirmation' };
...
export function operationalReviewKeyboardAction(
  ...
  if (input.confirmationOpen) {
    if (input.key === 'Escape') return { type: 'cancel-confirmation' };
    if (input.key === 'Enter') return { type: 'submit-confirmation' };
    return { type: 'none' };
  }
  ...
  if (input.key === 'Enter') return { type: 'open-confirmation' };

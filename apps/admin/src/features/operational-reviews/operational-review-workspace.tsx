...
export function OperationalReviewWorkspace({
  ...
  const refreshGames = useCallback(async () => {
    ...
    setSelectedGameId((current) =>
      result.games.some((game) => game.id === current)
        ? current
        : (result.games[0]?.id ?? ''),
    );
    setGamesState('ready');
  ...
  const refreshJobs = useCallback(async () => {
    ...
    setSelectedJobId((current) =>
      result.jobs.some((job) => job.id === current)
        ? current
        : (result.jobs[0]?.id ?? ''),
    );
    setJobsState('ready');
  ...
  return (
    <div className="operationalReviewContext" aria-label="Kontekst weryfikacji">
      ...
      <label>
        Import zdjęć
        <select
          disabled={jobsState === 'loading' || jobs.length === 0}
...
function OperationalReviewBoard({
  ...
  const changedCellCount = item.cells.filter(
  ...
  const canOpenConfirmation =
    ...
    !isSaving;
  const resolutionAction = sequenceIsValid
    ? operationalReviewResolutionAction(item, sequenceNumber, draftSymbols)
    : 'corrected';
  ...
  const submitResolution = useCallback(async () => {
    ...
    if (!result.ok) {
      ...
      if (result.isRevisionConflict) setConfirmOpen(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (confirmationWasOpenRef.current) {
      ...
      queueMicrotask(() => approvalButtonRef.current?.focus());
    }
  }, [confirmOpen]);
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      const action = operationalReviewKeyboardAction({
        ...
        key: event.key,
        otherDialogOpen:
          openDialog !== null && openDialog !== confirmationDialogRef.current,
        repeat: event.repeat,
        saving: isSaving,
        shortcuts,
      ...
      if (action.type === 'cancel-confirmation') {
        setConfirmOpen(false);
      } else if (action.type === 'submit-confirmation') {
        void submitResolution();
      } else if (action.type === 'previous') {
        onPrevious();
      } else if (action.type === 'next') {
        onNext();
      } else if (action.type === 'open-confirmation') {
        openConfirmation();
      } else {
  ...
  return (
    <>
          ...
          {item.cells.map((cell, index) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <strong>
                    {symbolByCode.get(draftSymbolCode)?.name ?? draftSymbolCode}
                  </strong>
                  <small>{formatOperationalConfidence(cell.confidence)}</small>
                </div>
                <span>
      ...
      <section className="operationalReviewSource">
        <header>
          <div>
            <p className="eyebrow">Kontekst źródłowy</p>
            <h2>Oryginalne zdjęcie</h2>
          </div>
          <code>{item.sourceChecksumSha256.slice(0, 16)}…</code>
        </header>
        <OperationalReviewImage
          alt={`Oryginalne zdjęcie układu ${displaySequence ?? 'bez numeru'}`}
          key={item.id}
          src={operationalReviewAssetUrl(
            apiBaseUrl,
            context,
      ...
      <p className="operationalReviewShortcutNote">
        ...
        naciśnij Enter. Dopiero drugi Enter w oknie potwierdzenia zapisze całą
        planszę. Escape anuluje.
      </p>

      {confirmOpen ? (
        <dialog
          aria-labelledby="operational-review-confirm-title"
          aria-modal="true"
          className="paylineDialog operationalReviewConfirmDialog"
          onCancel={(event) => {
            event.preventDefault();
        ...
        >
          <div className="paylineDialogCard operationalReviewConfirmCard">
            <header className="paylineDialogHeader">
              <div>
                <p className="eyebrow">Drugi krok zatwierdzenia</p>
                <h2 id="operational-review-confirm-title">
                  Zapisz cały układ #{sequenceNumber}?
                </h2>
              </div>
            </header>
            <p>
              Decyzja: <strong>{resolutionAction}</strong>. Zmienione komórki:{' '}
              <strong>{changedCellCount}</strong>. Zapis utworzy jedną rewizję
              powiązaną z bieżącą geometrią.
            </p>
            <p className="operationalReviewConfirmHint">
              Naciśnij ponownie <kbd>Enter</kbd>, aby zapisać, albo{' '}
              <kbd>Escape</kbd>, aby anulować.
            </p>
            <footer className="formActions">
              ...
              <button
                ...
                type="button"
              >
                {isSaving ? 'Zapisywanie…' : 'Potwierdź zapis'}
              </button>
            </footer>
          </div>
        </dialog>

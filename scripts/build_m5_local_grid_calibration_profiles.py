"""Build exact-source local-frame calibration profiles from accepted anchors."""
...
DEFAULT_OUTPUT = ROOT / "ai_docs" / "quality" / "m5-local-grid-calibration-profiles.json"
...
def main() -> int:
    ...
    try:
        ...
        content = local_profile_document_bytes(
            build_local_profile_document(
                review.golden,
        ...
        if args.check:
            if not args.output.exists() or args.output.read_bytes() != content:
                print(
                    "ERROR: local grid calibration profiles are missing or stale.",
                    file=sys.stderr,
                )
                return 1
        else:
        ...
        print(
            json.dumps(
                {
                    "coveredSourceImageCount": report["coveredSourceImageCount"],
                    "missingSourceImageCount": len(report["missingSourceImageChecksums"]),
        ...
        code = getattr(error, "code", "LOCAL_GRID_CALIBRATION_BUILD_FAILED")
        print(
            json.dumps(

"""Crop the M5 corpus from per-board detector geometry plus symbol refinement."""
...
DEFAULT_OUTPUT = (
    ROOT / "ai_docs" / "quality" / "m5-board-cell-crops-v6-detector-symbol-aware-affine-report.json"
)
...
def main() -> int:
    ...
    try:
        report = crop_detected_corpus(
            ...
            args.artifact_root,
            cropper=PerspectiveBoardCellCropperV6DetectorSymbolAwareAffine(),
        )
        content = report.to_json_bytes()
        if args.check:
            if not args.output.exists() or args.output.read_bytes() != content:
                raise BoardCropError(
                    "DETECTOR_SYMBOL_AWARE_CROP_REPORT_STALE",
                    "Detector symbol-aware crop report is missing or stale.",
                )
        else:
        ...
        code = getattr(error, "code", "DETECTOR_SYMBOL_AWARE_CROP_FAILED")
        print(
            json.dumps(

"""Crop all M5 boards using strict refinement plus accepted fallback overrides."""
...
from game_predictor_worker.images.symbol_grid_overrides import (  # noqa: E402
...
DEFAULT_REVIEW = ROOT / "artifacts" / "m5-symbol-grid-fallback-review" / "reviewed-geometry.json"
DEFAULT_OUTPUT = (
...
def main() -> int:
    ...
    try:
        ...
        report = crop_detected_corpus(
            ...
            args.artifact_root,
            cropper=PerspectiveBoardCellCropperV7ReviewedSymbolAwareAffine(),
        ...
        if args.check:
            if not args.output.exists() or args.output.read_bytes() != content:
                raise BoardCropError(
                    "REVIEWED_SYMBOL_AWARE_CROP_REPORT_STALE",
                    "Reviewed symbol-aware crop report is missing or stale.",
                )
        else:
            ...
            json.dumps(
                {
                    ...
                    "manualOverrideCount": overrides.override_count,
        ...
        code = getattr(error, "code", "REVIEWED_SYMBOL_AWARE_CROP_FAILED")
        print(
            json.dumps(

"""Generate conservative M5 crops from expanded per-board bounding frames."""
...
def main() -> int:
    ...
    try:
        report = crop_detected_corpus(
            ...
            args.artifact_root,
            cropper=SafeContextBoardCellCropper(),
        ...
        if args.check:
            if not args.output.exists() or args.output.read_bytes() != content:
                raise BoardCropError(
                    "SAFE_CONTEXT_CROP_REPORT_STALE",
                    "Safe-context crop report is missing or stale.",
                )
        else:
            ...
            json.dumps(
                {
                    ...
                    "croppedImageCount": value["croppedImageCount"],
                    "cropperVersion": value["cropperVersion"],
        ...
        code = getattr(error, "code", "SAFE_CONTEXT_CROP_FAILED")
        print(
            json.dumps(

"""Prepare and serve the strict symbol-grid fallback review queue."""
...
from game_predictor_worker.images.symbol_grid_fallback_review import (  # noqa: E402
...
DEFAULT_REFINEMENT_REPORT = (
    ROOT / "ai_docs" / "quality" / "m5-full-symbol-grid-refinement-detector-report.json"
)
DEFAULT_OUTPUT = ROOT / "artifacts" / "m5-symbol-grid-fallback-review" / "reviewed-geometry.json"
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--refinement-report", type=Path, default=DEFAULT_REFINEMENT_REPORT)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("prepare")
    serve = subparsers.add_parser("serve")
    serve.add_argument("--host", default="127.0.0.1")
    serve.add_argument("--port", type=int, default=8887)
    serve.add_argument("--no-open", action="store_true")
    return parser


def _review(args: argparse.Namespace) -> SymbolGridFallbackReview:
    return SymbolGridFallbackReview(
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "serve":
            ...
            print(f"Symbol-grid fallback review is ready: {url}")
            ...
            try:
                ...
                print("\nStopping symbol-grid fallback review.")
            finally:
                server.server_close()
            return 0
    except (
        ...
        code = getattr(error, "code", "SYMBOL_GRID_FALLBACK_REVIEW_FAILED")
        print(
            json.dumps(

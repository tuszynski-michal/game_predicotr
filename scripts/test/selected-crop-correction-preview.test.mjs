...
test('failure recovery previews only a missing persisted worker result and resumes safely', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'selected-crop-failure-'));
  const source = path.join(root, 'picked');
  const current = path.join(root, 'picked cut');
  const output = path.join(root, 'picked cut v12 failures preview');
  const state = path.join(current, '.manual-image-crop-state');
  const results = path.join(state, 'results');
  const fileName = 'seq_1-9.jpg';
  try {
        ...
        failures: [
          {
            fileName,
            stage: 'write',
            code: 'SELECTED_IMAGE_CROP_WORKER_TIMEOUT',
            failedAt: '2026-09-14T00:00:00.000Z',
          },
        ],
        preparationPolicyVersion:
          'selected-image-board-band-v14-vertical-lattice-guarded',
        updatedAt: '2026-09-14T00:00:00.000Z',
      }),
    );
    await writeFile(
      path.join(results, '000000.json'),
      JSON.stringify({ schemaVersion: 2, shardIndex: 0, results: {} }),
    );

    const first = await runSelectedCropFailureRecoveryPreview(
      source,
      current,
      output,
    );
    assert.deepEqual(
      ...
      { selection: 'missing_failures', total: 1, completed: 1, failures: 0 },
    );
    assert.ok((await readFile(path.join(output, fileName))).length > 0);

    const resumed = await runSelectedCropFailureRecoveryPreview(
      source,
      current,
      output,
    );

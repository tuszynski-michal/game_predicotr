...
class AutomaticPartialGeometryProposalPayload(ApiModel):
    ...
    def validate_proposal(self) -> AutomaticPartialGeometryProposalPayload:
        learned = self.policy_version == "structured-lattice-v4-lateral-partial-v2"
        ...
        expected_qualification_version = (
            "manual-geometry-qualification-v2" if learned else "manual-geometry-qualification-v1"
        )
        ...
        if (
            ...
            and self.policy_checksum_sha256 != LateralPartialGeometrySnapshot().checksum_sha256
        ):
            raise ValueError("The automatic partial policy checksum changed.")
        if learned != (self.training_profile_checksum_sha256 is not None):

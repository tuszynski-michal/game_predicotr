...
def _protected_bbox_in_ideal(
    ...
    *,
    ...
    left = (
        float(candidate.left) if candidate.left is not None else candidate.x - candidate.width / 2
    )
    top = float(candidate.top) if candidate.top is not None else candidate.y - candidate.height / 2
    points = np.asarray(
        [
            [left - margin_x, top - margin_y],
            [left + candidate.width + margin_x, top - margin_y],
            [left + candidate.width + margin_x, top + candidate.height + margin_y],
            [left - margin_x, top + candidate.height + margin_y],
        ],
        dtype=np.float64,
    )
...
def _detector_quad(
    ...
    return cast(
        tuple[DetectorPoint, DetectorPoint, DetectorPoint, DetectorPoint],
        tuple(DetectorPoint(point.x, point.y) for point in value.corners),
    )

"""Exact-observation review queue for the v14 full-preflight fallbacks."""
...
REVIEW_VERSION = "v14-projective-fallback-review-v1"
SELECTION_VERSION = "all-v14-full-preflight-fallbacks-v1"
SUGGESTION_VERSION = "detector-quad-manual-correction-v2"
REPORT_VERSION = "m5-projective-fixed-padding-full-preflight-v1"
CANDIDATE_VERSION = "v14-bbox"
...
def _sequence(value: object, label: str) -> Sequence[object]:
    if not isinstance(value, Sequence) or isinstance(value, str | bytes):
        raise CellGridGoldenError(
            "V14_FALLBACK_REVIEW_CONTRACT_INVALID",
...
def _fallbacks(report: Mapping[str, object]) -> dict[tuple[str, int], Mapping[str, object]]:
    if (
        report.get("schemaVersion") != REPORT_VERSION
        or report.get("phase") != "full"
        or report.get("candidate") != CANDIDATE_VERSION
        or report.get("trainingAllowed") is not False
    ):
        raise CellGridGoldenError(
            "V14_FALLBACK_REVIEW_REPORT_INVALID",
            "The v14 full-preflight report is unsupported.",
        )
    ...
    for index, raw in enumerate(_sequence(report.get("entries"), "preflightReport.entries")):
        ...
        if key in selected:
            raise CellGridGoldenError(
                "V14_FALLBACK_REVIEW_SELECTION_DUPLICATED",
                "The preflight report duplicates a fallback board.",
            )
        selected[key] = entry
    fallback_count = _integer(report.get("fallbackCount"), "preflightReport.fallbackCount")
    if len(selected) != fallback_count:
        raise CellGridGoldenError(
            "V14_FALLBACK_REVIEW_SELECTION_DRIFT",
            "Fallback count differs from the preflight report.",
        )
    if not selected:
        raise CellGridGoldenError(
            "V14_FALLBACK_REVIEW_NOT_REQUIRED",
            "The v14 full-preflight report contains no fallback boards.",
        )
    return selected


class V14ProjectiveFallbackReview(CellGridGoldenReview):
    """Reuse the perspective editor for only the v14 fail-closed boards."""
    ...
    def __init__(
        ...
        *,
        ...
        self.preflight_report_path = preflight_report_path.resolve(strict=True)
        ...
        if self.output_path == self.crop_root or self.output_path.is_relative_to(self.crop_root):
            raise CellGridGoldenError(
                "V14_FALLBACK_REVIEW_OUTPUT_IN_CROP_ROOT",
                "Fallback review data must be outside immutable board artifacts.",
            )
        report_bytes, report = _load_json(self.preflight_report_path, "preflightReport")
        self._preflight_report_sha256 = hashlib.sha256(report_bytes).hexdigest()
        ...
        candidates = {
            (candidate.source_image_checksum_sha256, candidate.board_position): candidate
            for candidate in bundle.candidates
        }
        missing = sorted(set(fallback_by_key) - set(candidates))
        if missing:
            raise CellGridGoldenError(
                "V14_FALLBACK_REVIEW_SOURCE_DRIFT",
        ...
        self._fallback_by_observation = {
            candidate.observation_id: fallback_by_key[
                (candidate.source_image_checksum_sha256, candidate.board_position)
            ]
        ...
        if self.output_path.exists():
            _, existing = _load_json(self.output_path, "v14ProjectiveFallbackReview")
    ...
    def _document(self) -> dict[str, object]:
        ...
        value["preflightReportSha256"] = self._preflight_report_sha256
        ...
        for entry in entries:
            fallback = self._fallback_by_observation[cast(str, entry["observationId"])]
    ...
    def _parse_existing(
        ...
        if (
            ...
            or value.get("preflightReportSha256") != self._preflight_report_sha256
            ...
            raise CellGridGoldenError(
                "V14_FALLBACK_REVIEW_SOURCE_DRIFT",
                "Existing v14 fallback review differs from its immutable sources.",
            )
        raw_entries = _sequence(value.get("entries"), "fallbackReview.entries")
        if len(raw_entries) != len(initial.entries):
            raise CellGridGoldenError(
                "V14_FALLBACK_REVIEW_SELECTION_DRIFT",
                "Existing v14 fallback review entry count differs.",
            )
        ...
        for index, (raw, expected) in enumerate(zip(raw_entries, initial.entries, strict=True)):
            ...
            if (
                ...
                raise CellGridGoldenError(
                    "V14_FALLBACK_REVIEW_SELECTION_DRIFT",
                    f"Existing v14 fallback review entry {index} differs.",
                )
